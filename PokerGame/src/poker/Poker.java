...
public class Poker {
	...
	public boolean isFullHouse(String cards[], int n) {
		return isThreeOfaKind(cards, n) && isTwoPairs(cards, n) &! isFourOfKind(cards,n);
	}
	...
	public boolean isFourOfKind(String cards[], int n) {
		for (int i=0; i<n-1; i++) {
			if (cards[i].charAt(1) == cards[i+1].charAt(1) && cards[i+1].charAt(1) == cards[i+2].charAt(1) && cards[i+2].charAt(1)== cards[i+3].charAt(1) ) {
				return true;
			}
		}
		return false;
	...
	public static void main(String args[]) {
		...
		System.out.println(new Poker().isFourOfKind(
				new String[] {"C6", "D6", "H6", "S6", "D7"}, 5));
	}
}
// end of Poker.java
